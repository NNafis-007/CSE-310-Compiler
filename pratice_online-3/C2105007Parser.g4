...
func_definition
	...
	| ts = type_specifier ID LPAREN p = parameter_list non = non_params {
...
compound_statement
    ...
    | SWITCH LPAREN e=expression RPAREN cs=compound_statement
        {
            int lineNo = $cs.Cstat_rri.lineNo;
            String cs_text = $cs.Cstat_rri.text;

            String sw_text = "switch(" + $e.Expr_rri.text + ")" + cs_text;
            System.out.println("FOUND SWITCH \n" + sw_text);

            wParserLog("Line " + lineNo + ": swtich_def : SWITCH LPAREN expression RPAREN cs\n");
            wParserLog(sw_text);
            $Cstat_rri = new RuleReturnInfo(lineNo, sw_text);
...
var_declaration
    ...
    | ID COLON {
        //label rule
        int lineNo = $COLON.getLine();
        String labelName = $ID.getText();
        LABELS.add(labelName);
        String text = labelName + ":\n";
        wParserLog("Line " + lineNo + ": LABEL START : LABEL COLON\n");
        wParserLog(text);
        System.out.println("LABEL DECLARATION DETECTED\n");
        $vdec_rri = new RuleReturnInfo(lineNo, text);

    }  
    | t=type_specifier a=array_init sm=SEMICOLON
        {
        wParserLog("Line " + $sm.getLine()
            +": var_declaration : type_specifier ARRAY_INIT SEMICOLON\n"
        );

        int lineNo = $sm.getLine();
        String text = $t.type_rri.text + " " + $a.arr_rri.text + ";";
        $vdec_rri = new RuleReturnInfo(lineNo, text);
        }
    ;
...
array_init returns [RuleReturnInfo arr_rri]:
    | ID LTHIRD RTHIRD ASSIGNOP il=init_list {
            //array decl
            int lineNo = $ID.getLine();
            String text = $ID.getText() + "[]=" + $il.il_rri.text;
            wParserLog("Line " + lineNo + ": array_init : ID LTHIRD RTHIRD ASSIGNOP init_list\n");
            wParserLog($text + "\n");
            $arr_rri = new RuleReturnInfo(lineNo, text);
        }
    ;

init_list returns [RuleReturnInfo il_rri]:
	LCURL a = arguments COMMA le = logic_expression RCURL {
            int lineNo = $COMMA.getLine();
            String text = "{" + $a.arg_rri.text + "," + $le.LogicExpr_rri.text + "}";
            System.out.println("STARTED INITIALIZATION");

            wParserLog("Line " + lineNo + ": init_list : LCURL arguments COMMA logic_expression RCURL\n");
            wParserLog(text + "\n");
            $il_rri = new RuleReturnInfo(lineNo, text);
        }
	| LCURL l = logic_expression RCURL {
            int lineNo = $l.LogicExpr_rri.lineNo;
            String text = "{" + $l.LogicExpr_rri.text + "}";
            System.out.println("STARTED INITIALIZATION");


            wParserLog("Line " + lineNo + ": init_list : {logic_expression}\n");
            wParserLog(text + "\n");
            $il_rri = new RuleReturnInfo(lineNo, text);
        }
;
...
statement returns[RuleReturnInfo stat_rri]:
	...
	| FOR {canBreak=true;} LPAREN es1 = expression_statement es2 = expression_statement e =
		expression RPAREN s = statement {
            int lineNo = $s.stat_rri.lineNo;
            ...
            $stat_rri = new RuleReturnInfo(lineNo, text);
            wParserLog("Line " + lineNo + 
                ": statement : FOR LPAREN expression_statement expression_statement expression RPAREN statement CONTROL stmt\n");
            wParserLog(text);
            canBreak=false;


        }
    | CASE {canBreak=true;} CONST_INT COLON ss = statements BREAK SEMICOLON
        {
            int lineNo = $CASE.getLine();
            String text = "case " + $CONST_INT.getText() + ":\n" + $ss.Stats_rri.text + "break;\n\n";

            System.out.println("Matched CASE statement with constant int at line " + lineNo);
            wParserLog("Line " + lineNo + ": statement (CASE) : CASE CONST_INT COLON statements BREAK SM\n");
            wParserLog(text);
            $stat_rri = new RuleReturnInfo(lineNo, text);
            canBreak = false; // reset canBreak after case statement
    ...
    | TIMES LPAREN e = expression RPAREN s = statement
        {
            int lineNo = $s.stat_rri.lineNo;
            String expr = $e.Expr_rri.text;
            String type = typeDetector(expr);
            if(!type.equals("int")){
                logErr("ERROR at line " + lineNo + ": Invalid Iteration count in times");
                System.out.println("ERROR at line " + lineNo + ": Invalid Iteration count in times");
            }
            String st = $s.stat_rri.text;
            String text = "times(" + expr + ")" + st;
            $stat_rri = new RuleReturnInfo(lineNo, text);
            System.out.println("---Handling Times loop---");
            wParserLog("Line " + lineNo + ": statement : TIMES LPAREN expression RPAREN statement\n");
            wParserLog(text);
	...
	| WHILE {canBreak=true;} LPAREN e = expression RPAREN s = statement {
            // while(a[0]--){
            //     c = c - 2;
            // }
    ...
    | DO{canBreak=true;} s=statement WHILE LPAREN e = expression RPAREN SEMICOLON
        {
            int lineNo = $SEMICOLON.getLine();
            String condns = $e.Expr_rri.text;

            String text = "do" + $s.stat_rri.text + "while" + "(" + condns + ");\n";
            $stat_rri = new RuleReturnInfo(lineNo, text);
            wParserLog("Line " + lineNo + ": statement : DO statement WHILE expression RPAREN SEMICOLON\n");
            wParserLog(text);
            canBreak=false;
	...
	| BREAK SEMICOLON {
            ...
            int lineNo = $SEMICOLON.getLine();
            String text = "break;\n";
            System.out.println("in loop? " + canBreak);

            if(canBreak == false){
                logErr("Error at line " + lineNo + ": break statement outside loop\n");
                System.out.println("Error at line " + lineNo + ": break statement outside loop\n");
            }

            wParserLog("Line " + lineNo + ": statement : BREAK SEMICOLON\n");
            wParserLog(text);
            $stat_rri = new RuleReturnInfo(lineNo, text);
        }
	| CONTINUE SEMICOLON {
            ...
            System.out.println("in loop? " + canBreak);
            if(canBreak == false){
                logErr("Error at line " + lineNo + ": continue statement outside loop\n");
                System.out.println("Error at line " + lineNo + ": continue statement outside loop\n");
            }


            wParserLog("Line " + lineNo + ": statement : CONTINUE SEMICOLON\n");
            wParserLog(text);
            $stat_rri = new RuleReturnInfo(lineNo, text);
        }
    | GOTO ID SEMICOLON
        {
            ...
            if (!LABELS.contains(label)){
                logErr("Error at line " + lineNo 
                    + " : Label not declared\n");
                System.out.println("Error at line " + lineNo 
                    + " : Label not declared\n");
            }
            String text = "goto " + label + ";\n";
            wParserLog("Line " + lineNo + ": statement : GOTO LABEL SEMICOLON\n");
            wParserLog(text);
...
expression
    ...
    | var = variable ASSIGNOP l = logic_expression QUE e1=expression COLON e2=expression
        {
                int lineNo = $l.LogicExpr_rri.lineNo;
                String var_name = $var.var_rri.text;
                ...
                String expr1 = $e1.Expr_rri.text;
                String expr2 = $e2.Expr_rri.text;
                
                wParserLog("Line " + lineNo + ": expression : TERNARY\n");

                String text = var_name + "=" + logic_expr + "?" + expr1 + " : " + expr2;
                System.out.println("Found ternary : " + text);
...
non_params
	returns[RuleReturnInfo non_param_rri]:
	LPAREN {
...
invalid_char
	returns[RuleReturnInfo invalid_rri]:
	ic = INVALID_CHAR {
